...
const breadcrumbNameMap = {
  ...
  "/dashboard/sms-templates/edit-sms-template/:id": "Edit",
...
const BreadcrumbComponent = ({ match, location }) => {
    ...
    return (
      <>
        {breadcrumbNameMap[url] && (
          <Breadcrumb.Item key={breadcrumbNameMap[url]}>
            <Link
              className="breadcrumb-link"
              to={{
                pathname: getUrl(url, match.params),
  ...
  return (
    <Breadcrumb separator="/" key={new Date().getTime()}>
      {breadcrumbItems}
    </Breadcrumb>
  );
};
