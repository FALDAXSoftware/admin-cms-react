...
class PageCounterComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      this.state.dataCount != nextProps.dataCount ||
        ...
        <span className="float-left">
          <Tooltip title="Sync Data">
            <Icon
              ...
              type="sync"
              onClick={this.props.syncCallBack}
            />
          </Tooltip>
        </span>
        {dataCount != 0 && (
          <span className="float-right">
            {(parseFloat(page) - 1) * parseFloat(limit) + 1}-
            {parseFloat(page) * parseFloat(limit) > parseFloat(dataCount)
              ? dataCount
              : parseFloat(page) * parseFloat(limit)}
            &nbsp;of&nbsp;{dataCount}
          </span>
        )}
      </div>
