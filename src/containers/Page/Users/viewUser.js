...
import UpgradeUserTier from "./upgradeUserTier";
      ...
      <LayoutWrapper>
        ...
        <Tabs className="full-width">
          ...
          {isAllowed("get_kyc_detail") && (
            <TabPane tab="Customer ID Verification" key="2">
              <UserKYCDetails user_id={user_id} />
            </TabPane>
          )}
          {isAllowed("get_user_wallet_addresses") && (
            <TabPane tab="Wallets" key="3">
              <UserWallets
                is_deleted={
          ...
          {isAllowed("get_user_login_history") && (
            <TabPane tab="Login History" key="5">
              <LoginHistory user_id={user_id} />
            </TabPane>
          )}
          {isAllowed("get_all_trade") && (
            <TabPane tab="History" key="6">
              <AllTrades user_id={user_id} />
            </TabPane>
          )}
          {/* {isAllowed("get_all_trade") && */}
          <TabPane tab="Tier Unlock" key="13">
            <UpgradeUserTier user_id={user_id} />
          </TabPane>
          {/* } */}
          {isAllowed("update_user_referal") && isAllowed("get_user_details") && (
            <TabPane tab="User Referral Percentage" key="7">
              <Referral user_id={user_id} />
            </TabPane>
          )}
          {isAllowed("referred_users") && (
            <TabPane tab="Referred Users" key="8">
              <ReferredUsers user_id={user_id} />
            </TabPane>
          )}
          {isAllowed("get_user_transactions") && (
            <TabPane tab="Transaction History" key="9">
              <UserTransactionHistory user_id={user_id} />
            </TabPane>
          )}
          {isAllowed("get_all_withdraw_request") && (
            <TabPane tab="Withdrawal Request" key="10">
              <UserWithdrawRequest user_id={user_id} />
            </TabPane>
          )}
