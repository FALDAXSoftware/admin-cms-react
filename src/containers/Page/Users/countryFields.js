...
export default class CountryFields extends Component {
    ...
    if (this.state.isUpdate) {
      if (this.props.selectedCountry != nextProps.countryName) {
        let country = this.state.countries.find(
          (country) => country.name == nextProps.countryName
        );
        if (country) {
          this.getStatesOfACountry(country.id);
        }
      }
      if (this.props.selectedState != nextProps.stateName) {
        let state = states.find((state) => state.name == nextProps.stateName);
        if (state) {
          this.getCitiesOfAState(state.id);
        }
      }
      this.setState({
        // states,
        // cities,
        selectedCountry: nextProps.countryName,
        selectedState: nextProps.stateName,
  ...
  getStatesOfACountry(id) {
    ...
    let _this = this;
    _this.setState({ loader: true });
    ApiUtils.getallStatesData(token, id)
      .then((res) => res.json())
      .then((res) => {
        this.setState({
          states: res.data,
  ...
  getCitiesOfAState(id) {
    ...
    let _this = this;
    _this.setState({ loader: true });
    ApiUtils.getallCitiesData(token, id)
      .then((res) => res.json())
      .then((res) => {
        this.setState({
          cities: res.data,
    ...
    this.state.countries.filter((country) => {
      if (country.name == value) {
        this.setState({ changedCountry: country }, () => {
          ...
          console.log("test", position.key);
          this.getStatesOfACountry(position.key);
          this.setState({
  ...
  _changeState = (value, position) => {
    ...
    var cities = this.getCitiesOfAState(position.key);
