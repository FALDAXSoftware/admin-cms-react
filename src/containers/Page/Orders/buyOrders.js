...
class BuyOrders extends Component {
  ...
  _getAllOrders = (isConvertToCsv=false) => {
    const { token, user_id } = this.props;
    const { searchOrder, page, limit, sorterCol, sortOrder } = this.state;
    let _this = this;

    _this.setState({ loader: true });
   (isConvertToCsv? ApiUtils.getAllBuyOrders(
      1,
      100000,
      token,
      "",
      ...
      .then(function(res) {
        if (res.status == 200) {
            ...
            _this.setState({
                allOrders: res.data,
                allOrderCount: res.buyBookCount
            });
            }
    ...
    return (
      <TableDemoStyle className="isoLayoutContent">
        ...
        <ExportToCSVComponent
          ...
          filename="buy_order.csv"
          data={csvData}
          header={exportSellOrder}
        />
