  ...
  render() {
    ...
    return (
      <Modal
        ...
        footer={[<Button onClick={this._closeViewRequestModal}>OK</Button>]}
      >
        <span>
          {" "}
          <b>Name:</b>{" "}
        ...
        <p style={{ marginBottom: "15px" }}>
          {twoFactorReqDetails.full_name
            ? twoFactorReqDetails.full_name
            : "N/A"}
        </p>

        {twoFactorReqDetails.uploaded_file &&
        twoFactorReqDetails.status == "open" ? (
          <div>
            <span>
              <b>Uploaded Image:</b>{" "}
            </span>
            <br />
            <a
              href={BUCKET_URL + twoFactorReqDetails.uploaded_file}
              target="_blank"
            >
              <RequestImg
                alt=""
                src={BUCKET_URL + twoFactorReqDetails.uploaded_file}
              />
            </a>
          </div>
        ) : (
