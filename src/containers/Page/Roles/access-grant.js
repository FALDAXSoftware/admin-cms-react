...
const AccessGrant = (props) => {
  ...
  const [tradeDeskPairs, setTradeDeskPairs] = useState([])
  ...
  const getRolePermissions = (roleId) => {
    ...
    ApiUtils.getRolePermissions(token, roleId).then((response) => response.json()).then((res) => {
      ...
      if (res.status == 200) {
        ...
        setTradeDeskPairs(res.roleValue.allowed_pairs ? res.roleValue.allowed_pairs.split(",") : [])
  ...
  const getPairs = () => {
    ApiUtils.getPairList().then((response) => response.json()).then((res) => {
      setPairs(res.data)
    })
  }
  ...
  const savePermissions = () => {
    ...
    let updatePermissionsRequest = {
      ...
      role_id: role.id,
      allowed_pairs: tradeDeskPairs.join(",")
    }
                    ...
                    {permissions[key].map((per, index) => (
                      <>
                        <Col span={6} key={index}>
                          ...
                          {per.display_name == "Trade Desk Management" && per.isChecked &&
                            <Select
                              style={{ width: "100%", marginTop: "10px" }}
                              mode="multiple"
                              placeholder="Select Allowed Pairs"
                              onChange={(value) => {
                                setTradeDeskPairs(value)
                              }}
                              value={tradeDeskPairs}>
                              {pairs.map((p) => (
