...
class BotConfig extends Component {
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Switch
              className="bot_config_switch"
              defaultChecked={record.bot_status}
              onChange={(value) => {
  ...
  save = (form, record) => {
    form.validateFields((error, row) => {
      ...
      ApiUtils.updatePairsForBot(this.props.token, {
        id: record.encript_id,
        min_crypto: row.crypto_minimum,
        max_crypto: row.crypto_maximum,
        flag: record.bot_status,
      })
  ...
  changeBotStatus = (value, record) => {
    ...
    ApiUtils.updatePairsForBot(this.props.token, {
      id: record.encript_id,
      min_crypto: record.crypto_minimum,
      max_crypto: record.crypto_maximum,
      flag: value,
    })
      .then((response) => response.json())
      .then((res) => {
  ...
  getData = () => {
    this.setState({ loader: true });
    ApiUtils.getPairsForBot(this.props.token)
      .then((response) => response.json())
      .then((res) => {
        console.log("-------------------", res);
        let data = [];
