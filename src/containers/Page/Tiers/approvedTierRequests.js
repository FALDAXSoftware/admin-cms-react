...
columns.push({
  ...
  render: (object) => (
    <span>
      ...
      <Button
        disabled={!(object["public_note"] || object["private_note"])}
        onClick={() =>
          self.setState({
            showNotesModal: true,
            public_note: object["public_note"],
            private_note: object["private_note"],
          })
        }
      >
        Show Notes
...
class ApprovedRequests extends Component {
  ...
  onExport = () => {
    this.setState({ openCsvExportModal: true }, () =>
      this.getAllApprovedTierRequest(true)
    );
  };
  render() {
    const {
    ...
    return (
      <div className="isoLayoutContent">
        ...
        <RejectNotesModal
          ...
          callback={(private_note, public_note) => {
            this.setState({ private_note: "", public_note: "" });
            this.forceApproveRejectTierRequest(
              false,
