...
import { tradingFeeTableInfos } from "../../Tables/antTables";
...
var self;
class FeesTrading extends Component {
  constructor(props) {
    super(props);
    this.state = {
      tradeFees: [],
  ...
  handleNetworkChange = (pagination, filters, sorter) => {
    ...
    if (sorter.columnKey == "value") {
      if (sorter.order == "descend") {
        coinFees = coinFees.sort(
          (a, b) =>
            parseFloat(a[sorter.columnKey]) - parseFloat(b[sorter.columnKey])
        );
      } else if (sorter.order == "ascend") {
        coinFees = coinFees.sort(
          (a, b) =>
            parseFloat(b[sorter.columnKey]) - parseFloat(a[sorter.columnKey])
        );
      }
    }
      ...
      <TableDemoStyle className="isoLayoutContent">
        ...
        {show2FAEnableModel && (
          <TwoFactorEnableModal
            ...
            onClose={this.on2FAEnableModalClose}
          />
        )}
        <TableWrapper
          rowKey="id"
          {...this.state}
          columns={tradingFeeTableInfos[0].columns}
          pagination={false}
          dataSource={tradeFees}
