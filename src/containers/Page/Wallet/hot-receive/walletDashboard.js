...
const columns = [
  {
    ...
    render: (object) => (
      <>
        {isAllowed("admin_hotreceive_wallet_details") && (
          <Tooltip className="btn-icon" title="View">
            <Icon
              onClick={() =>
                WalletWarmDashboard.navigateToView(object["coin_code"])
              }
              type="info-circle"
            ></Icon>
          </Tooltip>
...
class WalletWarmDashboard extends Component {
  ...
  static navigateToView = (coin_code) => {
    ...
    self.props.history.push({
      pathname: `./wallet/hotreceive/${coin_code}`,
  ...
  getWalletData = async (isExportToCsv = false) => {
    try {
      ...
      if (status == 200) {
        ...
        if (isExportToCsv) this.setState({ csvData: data });
