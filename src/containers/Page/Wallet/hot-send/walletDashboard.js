...
const columns=[
    {
       ...
       align:"left",
        key:3,
        width:100,
        render:object=>(<>{isAllowed("admin_hotsend_wallet_details") && <Tooltip className="btn-icon" title="View"><Icon onClick={()=>WalletWarmDashboard.navigateToView(object["coin_code"])} type="info-circle"></Icon></Tooltip>}</>)
    },
...
class WalletWarmDashboard extends Component {
    ...
    static navigateToView=(coin_code)=>{
        ...
        self.props.history.push({pathname:`./wallet/hotsend/${coin_code}`,state:{assets:JSON.stringify(assets)}})
    }
