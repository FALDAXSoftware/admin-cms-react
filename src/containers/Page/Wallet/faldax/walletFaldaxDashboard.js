...
const columns = [
  {
    ...
    render: (object) => (
      <>
        {isAllowed("send_coin_admin") && object["is_active"] == true && (
          <Tooltip className="cursor-pointer" title="Send">
            <Icon
              type="export"
              onClick={() => WalletFaldaxDashboard.openSendModal(object)}
            ></Icon>
          </Tooltip>
        )}
        {isAllowed("admin_faldax_wallet_details") && (
          <Tooltip className="btn-icon" title="View">
            <Icon
              onClick={() =>
                WalletFaldaxDashboard.navigateToView(object["coin_code"])
              }
              type="info-circle"
            ></Icon>
          </Tooltip>
  ...
  {
    title: (
      <IntlMessages id="walletFaldaxDashboardTable.title.total_earned_from_forfeit" />
    ),
    align: "left",
    key: 2,
  ...
  {
    title: (
      <IntlMessages id="walletFaldaxDashboardTable.title.total_earned_from_wallets" />
    ),
    align: "left",
    key: 4,
  ...
  {
    title: (
      <IntlMessages id="walletFaldaxDashboardTable.title.receive_address" />
    ),
    align: "left",
    key: 8,
...
class WalletFaldaxDashboard extends Component {
  ...
  getWalletData = async (isExportToCsv = false) => {
    try {
      ...
      let res = await (
        await (isExportToCsv
          ? ApiUtils.getAllWallets(this.props.token)
          : ApiUtils.getAllWallets(this.props.token, searchData))
      ).json();
  ...
  sendWalletBal = async () => {
    ...
    if (this.validator.allValid()) {
      try {
        this.loader.show();
        let res = await (
          await ApiUtils.sendWalletBalance(token, formData)
        ).json();
  ...
  getAssetAvailableBalance = async (asset, values) => {
    try {
      ...
      let res = await (
        await ApiUtils.getAvailableBalance(this.props.token, asset)
      ).json();
