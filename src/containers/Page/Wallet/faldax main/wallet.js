...
import { exportFaldaxMainWallet } from '../../../../helpers/exportToCsv/headers';
...
const columns = [
    {
        ...
        render: object => (
            <>
                {isAllowed("admin_get_bussiness_wallet_data") && <Tooltip className="btn-icon" title="View"><Icon onClick={() => WalletFaldaxDashboard.navigateToView(object["coin_code"])} type="info-circle"></Icon></Tooltip>}
            </>
        )
    },
    {
    ...
    {
        title: <IntlMessages id="walletFaldaxDashboardTable.title.balance" />,
        align: "left",
        key: 1,
        width: 100,
        dataIndex:"balance",
...
class WalletFaldaxDashboard extends Component {
    ...
    static navigateToView = (coin_code) => {
        ...
        self.props.history.push({ pathname: `./wallet/faldax-main-wallet/${coin_code}`, state: { assets: JSON.stringify(assets) } })
    }
    ...
    getWalletData = async (isExportToCsv = false) => {
        try {
            ...
            let res = await (await (isExportToCsv ? ApiUtils.getFaldaxMainWallet(this.props.token) : ApiUtils.getFaldaxMainWallet(this.props.token, searchData))).json();
