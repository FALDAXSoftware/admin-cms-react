...
class ApprovedKYC extends Component {
  ...
  static viewKYC(value, mtid, first_name, last_name, email, direct_response, kycDoc_details,
    webhook_response, address, country, city, zip, dob, id_type, created_at, front_doc, back_doc, ssn_number) {
    let kycDetails = {
      ...
      .then(function (res) {
        if (res.status == 200) {
          if (isExportToCsv) {
            ...
            _this.setState({ allKYCData: res.data, allKYCCount: parseInt(res.KYCCount), kycDetails: res.data });
          }
