...
const renderCell = (
  ...
  const front_doc = object[frontDoc];
  const back_doc = object[backDoc];
  const ssn = object[ssnNumber];

  switch (type) {
    case "KYCActionCell":
      return KYCActionCell(
        value,
        mtid,
        first_name,
        last_name,
    ...
    case "KYCReviewActionCell":
      return KYCReviewActionCell(
        value,
        mtid,
        first_name,
        last_name,
...
const ReviewKYCInfos = [
  {
    title: "Customer ID",
    value: "KYCTable",
    columns1: clone(columns1),
  },
];

export { columns, columns1, ApprovedKYCInfos, ReviewKYCInfos };
